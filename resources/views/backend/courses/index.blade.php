...
@section('content')
    <div class="row" style="margin-bottom: 4%;">
        <div class="col-sm-3 d-none" id="addbuttondiv">
            <a class="btn btn-block  btn-primary btn-rounded" href="{{ route('courses.create') }}">Add New Course</a>
        </div>
        @if (session('success'))
                                ...
                                @if (!empty($courses))
                                    @foreach ($courses as $val)
                                        <tr>
                                            ...
                                            <td> @if ($val->image)
                                                <img src="{{ asset('course/' . $val->image) }}"
                                                    alt="{{ $val->image }} " style="height:50px;">
                                            @endif</td>
                                            <td class="d-flex justify-content-around">
                                                <form action="{{ route('courses.update', ['course' => $val->id]) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    ...
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-footer">
                                                                ...
                                                                <form
                                                                    action="{{ route('courses.delete', ['course' => $val->id]) }}"
                                                                    method="POST">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <button type="submit"
